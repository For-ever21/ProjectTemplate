<!--
 * @Author         : Qiao
 * @Date           : 2021-03-23 16:55:07
 * @LastEditors    : Qiao
 * @LastEditTime   : 2021-04-26 10:33:32
 * @FilePath       : /src/modules/wax/components/addPlan/topicSelect.vue
 * @Description    : 话题选择
-->
<template>
...
<script lang="ts">
  ...
  import { getTopicWord } from "@/modules/wax/api/advPlan";
  @Component({
  ...
  export default class FanAccountSelect extends Vue {
    ...
    set select(val) {
      this.$emit("update:data", val.join("<_>"));
    }
    private mounted() {
    ...
    private packageData(res: Array<string>) {
      ...
      res.forEach((item: string) => {
        ...
        this.list.push({ name: arr[0], number: arr[1], id: item });
      });
      this.getCurrentSecondNnm();
    }
    private next() {
    ...
    private checkItem(id: any) {
      ...
      this.getCurrentSecondNnm();
      this.$emit("update:data", arr.join("<_>"));
    }
    ...
    private handleCheckAllChange(val: any) {
      ...
      if (val) this.$emit("update:data", addArr.join("<_>"));
      else this.$emit("update:data", resetArr.join("<_>"));
